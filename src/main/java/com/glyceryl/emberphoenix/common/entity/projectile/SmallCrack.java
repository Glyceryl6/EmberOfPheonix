...
public class SmallCrack extends AbstractHurtingProjectile {
    ...
    //制造一个假的爆炸效果
    public void createFireAndParticles(int radius) {
        ...
        for(BlockPos blockPos : BlockPos.withinManhattan(this.blockPosition(), radius, radius, radius)) {
            ...
            if (this.random.nextInt(6) == 0 && this.level.getBlockState(blockPos).isAir() && hasSolidBlock) {
    ...
    //模仿原版的爆炸，并对特定的实体进行伤害
    public void hurtParticularEntity(float radius) {
